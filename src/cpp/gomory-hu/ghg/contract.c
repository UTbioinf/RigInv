...
void setUnionContract(graph *g, node *v, node *w)
{
  ...
  printf("Entered setUnionContract\n");
#endif
#ifndef NO_PR
  v->age += 1;
#endif
     ...
     /* delete self-loops */
{
  ...
  if (( v->cap < g->minCap ) && (!(v->contracted)) && (g->uncontractedN > 1))
...
void compactContract(graph *g, node *v, node *w, int type)
{
  ...
  if (type == MERGE_FINISH) goto FINISH; 

  assert(v->leader == v);
  assert(w->leader == w);
  assert(g->currentN > 2);
  assert(g->uncontractedN >= 2);
  ...
  if (g->source != NULL)  /* flow has started */
    {
      ...
      if ((g->currentN <= 2) || (g->uncontractedN <= 1)) return;
    }
#endif

    if (!(w->contracted))
    {
      if (v->contains_uncontracted)
      {
        g->uncontractedN--;
        assert(g->uncontractedN > 0);
      }
      else /* v->contains_uncontracted == 0 */
      {
