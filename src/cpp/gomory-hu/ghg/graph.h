...
{
  weight_t        ORIGCAP;         /* original capacity */ 
...
typedef struct nodeSt
{
  ...
  arc             *afterlast;       /* for restore purposes */

  arc             *origfirst;       /* first outgoing arc to orig-array */
  arc             *origlast;        /* last outgoing arc to orig-array */
  arc             *origafterlast;       /* for restore purposes */

  struct nodeSt  *next;         /* used in compact.c */
  struct nodeSt  *up;           /* used in compact.c */
  struct nodeSt  *downleft;     /* used in compact.c */
  struct nodeSt  *downright;    /* used in compact.c */
  struct nodeSt  *firstN;       /* pointer to first node */
  int currentNinlist;           /* to update g->currentN */

  struct treenodeSt  *intree;       /* points to the nodes
                                       in tree it corresponds to */

  int contracted;               /* flag for contracted/uncontr. vertex */
  int uncontracted_count;       /* for use in random source/sink */
  int contains_uncontracted;    /* for contraction purposes, to indicate
                                   the right g->uncontractedN */

  int compact;                      /* depth of compacted nodes */

  int name;                         /* initial node name */ 

  struct nodeSt   *g_to_h;          /* correspondence links between
                                       two graphs */
  ...
  unsigned int     toTransfer:1;    /* for restartCutComp */
#endif  /* HO */
  ...
  unsigned int     in_2cut/*:1*/;       /* for multi-loop heuristic */
#endif
} node;
...
typedef struct graphSt
{
  int      n,                  /* number of vertices */ 
           ...
           uncontractedN;      /* uncontracted number of nodes */
  ...
  arc      *origarcs,          /* array of original (uncontracted) arcs */
           *sentinelOrigArc;   /* next after last original arc */

  arc      *FirstEmptyArc;     /* beginning of list of empty edges */
  arc      *LastEmptyArc;      /* end of list of empty edges */

  arc      *FirstEmptyOrigArc; /* beginning of list of empty orig edges */
  arc      *LastEmptyOrigArc;  /* end of list of empty orig edges */


  node     *Active,            /* pointer to the list of active nodes */
           *EmptyFirst,        /* pointer to the empty to_be_contracted_node list */
           *EmptyLast,
           *Last;              /* last contracted node */
  ...
  weight_t  min2Cap;           /* for multi-loop heuristic */
  weight_t  bestminCap;        /* best minimum cut capacity seen */
  float     ratio;             /* ratio of cut saved */
  int       bestcutcount;      /* count number of improved cuts */
  int       totalcutcount;     /* count number of smaller cuts */
  int       mincutcount;       /* count number of saved cuts */

  /* statistics */
  int       decr;              /* number of decreasing cuts in each iteration */
  ...
  float     calctimetotal;     /* time to find the mincuts */
  float     calctimemarker;    /* time to find the mincuts */
  float     cuttimetotal;      /* time to manipulate the cuts */
  float     cuttimemarker;     /* time to manipulate the cuts */
  int       iter;              /* total number of restartCutComps */
  int       maxiter;           /* maximum number of restartCutComps */
  int       mincuts;           /* counts number of non-trivial mincuts */
  int       mincuts_on;        /* flag for non-trivial mincuts */
...
#define OldForAllVertices(g, v) for (v=g->vertices; v!=g->sentinelVertex; v++)
#define ForAllVertices(g, v) for (v=g->Active; v!=g->sentinelVertex; v=v->next)
#define ForAllIncidentArcs(v,a) for ( a = v->first; a != NULL; a = a->next )
#define ForAllIncidentOrigArcs(v,a) for ( a = v->origfirst; a != NULL; a = a->next )
...
#define ForAllOrigArcs(g, a)  for (a=g->origarcs; a!=g->sentinelOrigArc; a++)
...
#define ForAllNodes(g, v) for (v=g->Active; v!=g->sentinelVertex; v=v->next)
#define ForAllArcs(g, a)  for (a=g->arcs; a!=g->sentinelArc; a++)
