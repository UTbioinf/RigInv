...
graph *makeGraph(int n, int m)
{
  ...
  memassert(g->vertexalloc    = (node*)calloc(10*g->n+1, sizeof(node)));
...
#endif
#endif
  memassert(g->arcs = (arc*)calloc(( 2*g->m + 8*g->n), sizeof(arc) ));
  g->sentinelArc = g->arcs + 2*g->m;

/* origarcs will always contain a backup copy of the arcs
   which may get deleted by node-contractions */
  memassert(g->origarcs = (arc*)calloc(( 2*g->m + 8*g->n), sizeof(arc) ));
  g->sentinelOrigArc = g->arcs + 2*g->m;
...
void saveTrivialCut(graph *g, node *v, weight_t val)
{
  ...
  ForAllVertices(g, w)
    count++;
  g->bestminCap = g->minCap;
  g->ratio = 1.0/(1.0 * (count - 1));
  g->bestcutcount++;
	  ...
	  while ( tail != i )
	    ...
	    { arc_new_num  = arc_first[tail];
		 ...
		 swapping these arcs:                                 */
	      ...
	      arc_new -> ORIGCAP      = arc_current -> CAPFIELD;
	      arc_current -> CAPFIELD = cap;
	      arc_current -> ORIGCAP  = cap;
      ...
      ForAllIncidentArcs ( v, a ) 
	{
	  ...
	  if ( w == v || w== NULL )  /* delete self-loops, 
	      ...
	      else 
		{
		  ...
		  b->ORIGCAP = b->CAPFIELD;
		  Reverse(b)->CAPFIELD +=  Reverse (a)->CAPFIELD;
		  Reverse(b)->ORIGCAP =  Reverse (b)->CAPFIELD;
