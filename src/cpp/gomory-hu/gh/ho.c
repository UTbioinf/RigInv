...
static void initCutComp(graph *g, node *source, node *sink)
{
  ...
  index1 = (random() % counter);
  index2 = (random() % counter);
...
static void initContracted(graph *g)
{
  ...
  g->Active = g->vertices;
  /** g->currentN should be set at this point **/
...
void computeCuts(graph *g)
{
  ...
  while ( g->Active != NULL )
  {
    ...
    else
    {
      ...
      initCutComp(g, g->Active, g->Active->next);
      while (g->currentN > 1)
      {
        STCut(g);
      ...
      aside = findEdges(g, g->Active);
      bside = findEdges(g, g->Last->up->downright->firstN);
      totaledges += (aside + bside);
