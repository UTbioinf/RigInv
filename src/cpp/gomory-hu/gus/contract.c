     ...
     /* delete self-loops */
{
  ...
  if (( v->cap < g->minCap ) && (!(v->contracted)))
...
void compactContract(graph *g, node *v, node *w, int type)
{
  ...
  {
    g->uncontractedN--;
    printf("g->uncontractedN is now %d\n", g->uncontractedN);
  }
...
/* the following if-stm was added because otherwise there is a problem
when w runs out of neighbors... */
